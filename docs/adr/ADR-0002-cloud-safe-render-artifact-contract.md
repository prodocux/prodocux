...
3. Kernel render I/O uses `artifact://` identities plus bounded inline bytes.
   Hosts inject `ArtifactResolverPort` and `ArtifactSinkPort`. The sink assigns
   output URIs (create-if-absent; conflicting digest fails closed). Callers
   supply only `output_name` and `delivery_mode`.
